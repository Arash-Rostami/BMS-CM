...
function dump_array_content($array, $indent) {
    ...
    foreach ($array as $key => $value) {
        ...
        if (is_array($value)) {
            ...
            if (is_string($value)) {
                // Ensure correct escaping. The str_replace from before might be simple, but var_export is safer mostly.
                // We'll use a custom string exporter.
...
foreach ($translations as $file => $keysMap) {
    foreach ($langs as $lang) {
        ...
        if (file_exists($path)) {
            ...
            $data = [];
            // Try to load 'en' as base if missing entirely
