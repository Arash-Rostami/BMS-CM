<div class="mb-6 sm:mb-8">
    <div class="glass border border-white/10 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden card-3d">
        <a href="{{ route('filament.dashboard.pages.dashboard') }}" target="_blank" rel="noopener noreferrer" class="block group relative">
            ...
            <div class="flex items-center justify-between relative z-10 gap-3 px-4 sm:px-6 py-3 sm:py-1.5 border-b border-slate-100/10 bg-slate-50/5 dark:bg-white/5">
                <div class="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                    <div class="w-5 h-5 sm:w-6 sm:h-6 bg-gradient-to-br from-cyan-500 to-blue-600 rounded flex items-center justify-center shadow-md flex-shrink-0">
                    ...
                    <p class="text-[11px] sm:text-xs truncate font-medium tracking-wide" :class="darkMode ? 'text-slate-400' : 'text-slate-500'">
                        {{ __('dashboard/strings.return_to_main_panel') }}
                    </p>
                </div>
                <img src="{{ asset('img/logos/curves.png') }}" class="w-20 sm:w-24 h-auto opacity-75 group-hover:opacity-100 transition-opacity duration-300" alt="logo">            </div>
        </a>
        <div class="px-6 py-2 sm:py-4 flex justify-center bg-slate-50/50 dark:bg-slate-900/20">
            <div class="inline-flex items-center p-2.5 sm:p-3.5 rounded-2xl sm:rounded-[1.5rem] bg-slate-200/50 dark:bg-white/5 border border-slate-300/50 dark:border-white/10 shadow-inner backdrop-blur-md gap-1.5 sm:gap-2">
