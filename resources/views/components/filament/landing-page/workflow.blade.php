@php
    $translations = [
        "dashboard.strings.attendance" => __("dashboard/strings.attendance") ?? "Attendance",
        "dashboard.strings.company_news" => __("dashboard/strings.company_news") ?? "Company News",
        "dashboard.strings.profile" => __("dashboard/strings.profile") ?? "Profile",
        "dashboard.strings.settings" => __("dashboard/strings.settings") ?? "Settings",
        "dashboard.strings.purchase_requests" => __("dashboard/strings.purchase_requests") ?? "Purchase Requests",
        "dashboard.strings.proforma_invoices" => __("dashboard/strings.proforma_invoices") ?? "Proforma Invoices",
        "dashboard.strings.view_orders" => __("dashboard/strings.view_orders") ?? "Registered Orders",
        "dashboard.strings.banks" => __("dashboard/strings.banks") ?? "Bank Profiles",
        "dashboard.strings.purchase_orders" => __("dashboard/strings.purchase_orders") ?? "Purchase Orders",
        "dashboard.strings.payments" => __("dashboard/strings.payments") ?? "Payments",
        "dashboard.strings.shipments" => __("dashboard/strings.submodules.shipment.title") ?? "Shipments",
        "dashboard.strings.customs" => __("dashboard/strings.submodules.custom_clearance.title") ?? "Customs",
        "dashboard.strings.edit_workspace" => __("dashboard/strings.edit_workspace") ?? "Edit Workspace",
        "dashboard.strings.back_to_standard" => __("dashboard/strings.back_to_standard") ?? "Reset to Default",
        "dashboard.strings.done" => __("dashboard/strings.done") ?? "Done",
        "dashboard.strings.workspace" => __("dashboard/strings.workspace") ?? "Workspace",
        "dashboard.strings.corporate_pillars" => __("dashboard/strings.corporate_pillars") ?? "Corporate Pillars",
    ];

    $availableModules = [
        ["id" => "purchaseRequests", "route" => route("filament.dashboard.resources.purchase-requests.index"), "icon" => "heroicon-o-document-text", "labelKey" => "dashboard.strings.purchase_requests", "color" => "blue"],
        ["id" => "proformaInvoices", "route" => route("filament.dashboard.resources.proforma-invoices.index"), "icon" => "heroicon-o-document-magnifying-glass", "labelKey" => "dashboard.strings.proforma_invoices", "color" => "indigo"],
        ["id" => "registeredOrders", "route" => route("filament.dashboard.resources.registered-orders.index"), "icon" => "heroicon-o-document-check", "labelKey" => "dashboard.strings.view_orders", "color" => "green"],
        ["id" => "bankProfiles", "route" => route("filament.dashboard.resources.bank-profiles.index"), "icon" => "heroicon-o-building-office", "labelKey" => "dashboard.strings.banks", "color" => "emerald"],
        ["id" => "purchaseOrders", "route" => route("filament.dashboard.resources.purchase-orders.index"), "icon" => "heroicon-o-shopping-bag", "labelKey" => "dashboard.strings.purchase_orders", "color" => "amber"],
        ["id" => "payments", "route" => route("filament.dashboard.resources.payments.index"), "icon" => "heroicon-o-banknotes", "labelKey" => "dashboard.strings.payments", "color" => "orange"],
        ["id" => "shipments", "route" => route("filament.dashboard.resources.shipments.index"), "icon" => "heroicon-o-truck", "labelKey" => "dashboard.strings.shipments", "color" => "purple"],
        ["id" => "customs", "route" => route("filament.dashboard.resources.customs.index"), "icon" => "heroicon-o-clipboard-document-check", "labelKey" => "dashboard.strings.customs", "color" => "violet"],
    ];
@endphp
    <div class="mb-10">
        <h2 class="text-xl sm:text-2xl font-bold mb-4" :class="darkMode ? 'text-white' : 'text-slate-900'">
            {{ $translations['dashboard.strings.corporate_pillars'] }}
        </h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <a href="#" class="glass border border-white/10 rounded-xl p-4 flex flex-col items-center justify-center gap-3 transition-all hover:-translate-y-1 hover:shadow-lg group">
                ...
                <span class="text-sm font-semibold text-center" :class="darkMode ? 'text-slate-300' : 'text-slate-700'">{{ $translations['dashboard.strings.attendance'] }}</span>
            </a>
            <a href="#" class="glass border border-white/10 rounded-xl p-4 flex flex-col items-center justify-center gap-3 transition-all hover:-translate-y-1 hover:shadow-lg group">
                ...
                <span class="text-sm font-semibold text-center" :class="darkMode ? 'text-slate-300' : 'text-slate-700'">{{ $translations['dashboard.strings.company_news'] }}</span>
            </a>
            <a href="{{ route('filament.dashboard.pages.dashboard') }}" class="glass border border-white/10 rounded-xl p-4 flex flex-col items-center justify-center gap-3 transition-all hover:-translate-y-1 hover:shadow-lg group">
                ...
                <span class="text-sm font-semibold text-center" :class="darkMode ? 'text-slate-300' : 'text-slate-700'">{{ $translations['dashboard.strings.profile'] }}</span>
            </a>
            <a href="#" class="glass border border-white/10 rounded-xl p-4 flex flex-col items-center justify-center gap-3 transition-all hover:-translate-y-1 hover:shadow-lg group">
                ...
                <span class="text-sm font-semibold text-center" :class="darkMode ? 'text-slate-300' : 'text-slate-700'">{{ $translations['dashboard.strings.settings'] }}</span>
            </a>
        </div>
    </div>
    ...
    <div x-data="{
        ...
        customized: false,
        isEditing: false,
        available: {{ json_encode($availableModules) }},
        ...
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-xl sm:text-2xl font-bold" :class="darkMode ? 'text-white' : 'text-slate-900'" x-text="translations['dashboard.strings.workspace']"></h2>
            <button @click="isEditing = true" class="text-sm font-semibold hover:underline" :class="darkMode ? 'text-cyan-400' : 'text-indigo-600'" x-text="translations['dashboard.strings.edit_workspace']"></button>
        </div>
            ...
            <template x-for="item in shortcuts" :key="item.id">
                <a :href="item.route" target="_blank" rel="noopener noreferrer" class="glass border border-white/10 rounded-2xl p-5 relative overflow-hidden group shadow-lg transition-all hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10 flex flex-col items-center text-center gap-3">
                        ...
                        <span class="font-semibold text-sm" :class="darkMode ? 'text-white' : 'text-slate-800'" x-text="translations[item.labelKey]"></span>
                    </div>
        ...
        <div x-show="isEditing" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            <div class="glass border border-white/10 rounded-3xl p-6 sm:p-8 relative z-10 w-full max-w-4xl max-h-full overflow-y-auto shadow-2xl flex flex-col">
                <h3 class="text-2xl font-bold mb-6" :class="darkMode ? 'text-white' : 'text-slate-900'" x-text="translations['dashboard.strings.edit_workspace']"></h3>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-8 flex-1 overflow-y-auto min-h-[50vh]">
                    <template x-for="module in available" :key="module.id">
                        <button @click="toggleShortcut(module)" class="relative glass border rounded-2xl p-4 flex flex-col items-center text-center gap-3 transition-all hover:scale-105" :class="selectedIds.includes(module.id) ? (darkMode ? 'border-cyan-400 bg-cyan-400/10' : 'border-indigo-600 bg-indigo-600/10') : 'border-white/10 opacity-70'">
                            ...
                            <span class="font-semibold text-sm" :class="darkMode ? 'text-white' : 'text-slate-800'" x-text="translations[module.labelKey]"></span>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-white/10 mt-auto">
                    <button @click="resetToDefault()" class="px-6 py-3 rounded-xl font-semibold transition-colors" :class="darkMode ? 'text-red-400 hover:bg-red-400/10' : 'text-red-600 hover:bg-red-600/10'" x-text="translations['dashboard.strings.back_to_standard']"></button>
                    <button @click="saveShortcuts()" class="px-8 py-3 rounded-xl font-semibold text-white shadow-lg transition-transform hover:scale-105" :class="darkMode ? 'bg-cyan-500 hover:bg-cyan-400' : 'bg-indigo-600 hover:bg-indigo-500'" x-text="translations['dashboard.strings.done']"></button>
                </div>
            </div>
        </div>
    </div>
